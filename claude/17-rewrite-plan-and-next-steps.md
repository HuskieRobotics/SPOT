...
- Move this `claude/` folder to `docs/spec/` in the `v6` tree (and leave a copy on `main`),
...
1. **Build the behavioral oracle from the legacy app.** This is the single highest-value step.
   ...
   - Run the *current* pipeline headlessly in Node against those fixtures (the CommonJS
   ...
   **Status 2026-09-17: the oracle exists.** `tools/oracle/run.js` (no dependencies) with
   ...
   | 2026 | `2026mnwi_official`, code `HEAD`, repo configs, TBA enrichment + OPR on | 435 TMPs, 48 teams, 74 entries, 0 errors, **32 unknown action ids** (see below) |

   Golden files: `tools/oracle/golden/<event>-<checkout>/{teams,tmps,report}.json` (3 MB and
   9 MB). Regenerate with `tools/oracle/run-all.sh <v4.2.0 worktree>`; see
   ...
   **Two-season oracle (decided 2026-09-17).** The oracle MUST cover **both 2026 and 2025**:
   ...
   - **Harness design (version-independent).** A standalone Node script kept outside the
     ...
     2026 oracle from `HEAD`, so both seasons use one tool. The legacy code needs no server,
   ...
   - **Semantic divergences between `v4.2.0` and current transformers** (verified with
     ...
     *current* semantics; the 2025 golden file records any expected difference.
   ...
   | Input | 2026 | 2025 |
   |-------|------|------|
   | Code checkout | `HEAD` of `main` (v5.2.x) | `v4.2.0` |
...
6. **Scaffold the `v6` branch**: Next.js (App Router, TypeScript), Tailwind + shadcn with a
   ...
   MongoDB memory server for tests, GitHub Actions running all of it on every PR, Dockerfile,
   `.env.example`, `deploy/aws` and `deploy/gcp` placeholders. Merge this as the first PR so
