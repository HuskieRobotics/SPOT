...
# =============================================================================
# SPOT — Deploy to Google Cloud Run
#
# Prerequisites:
#   - gcloud CLI (v410.0+) installed and authenticated (gcloud auth login)
#   - A GCP project with billing enabled (gcloud config set project <PROJECT_ID>)
#   - Your account must be a project Owner
#
# Usage:  cd deploy/gcp && chmod +x deploy.sh && ./deploy.sh
#
# After first deploy, visit the Cloud Run URL → /setup to configure the app.
# =============================================================================
...
for cmd in gcloud gsutil; do
  command -v "$cmd" &>/dev/null || { echo "ERROR: $cmd not found. Install Google Cloud SDK."; exit 1; }
done
...
# On brand new projects, Owner role propagation can be delayed.
# These explicit grants ensure the deploy works immediately.
