...
- With `idPrefixing: "phase"`, the recorded id is `prefix + button.id`, where the prefix is the
  selected kind's prefix plus the repeat index (`activeShift2`) if the scouter has toggled a
  kind, else the segment's own `prefix` (`teleopTransition`), else the phase's `prefix`
  (`teleop`, `endgame`). Pre-match (before start) the prefix is `""`.
