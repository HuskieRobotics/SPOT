...
| CS-2  | **Phases are a list, not a map keyed by time strings.** Each phase has `id`, `label`, `startMs`, optional `layer`, `prefix`, optional `pauseMs`, optional `variables`/`always`/`conditional` (2023 transition semantics kept).                                                                                                       | CF-4, CF-5, BL-298            |
| CS-3  | **Composite ids stay** (`idPrefixing: "phase"`): recorded id = prefix + button id, exactly as SC-23. `idPrefixing: "none"` reproduces pre-2026 behavior for archived configs. Prefixes are explicit per phase instead of being re-derived from `displayText` at runtime.                                                             | A-22, SC-23                   |
| CS-4  | **Shifts and endgame are configuration**, not client constants: `timing.shifts {intervalMs, startMs, endMs, maxIndex, kinds[{id, prefix, toggleButton}]}` and `timing.endgame {startMs, prefix, label}`.                                                                                                                             | SC-22, A-24                   |
| CS-5  | **Known action ids are derived**, never listed: `(phase prefixes ∪ endgame prefix ∪ shift prefixes 1..maxIndex ∪ "") × action-button ids ∪ extraActionIds`. The hidden catalog layer is gone. `extraActionIds` exists only for ids that data can contain but the grid cannot produce (TBA synthetic ids, hand-typed legacy ids).     | CF-9, A-23, F-4               |
...
```jsonc
{
  ...
  "timing": {
    ...
    "endgame": { "startMs": 30000, "prefix": "endgame", "label": "Endgame" },
    "shifts": {
      "intervalMs": 25000,
      "startMs": 130000,
      "endMs": 30000,
...
- With `idPrefixing: "phase"`, the recorded id is `prefix + button.id` where the prefix is the
  active shift prefix (`activeShift2`) while a shift is active, the endgame prefix once
  `endgame.startMs` is reached, and otherwise the phase prefix. Pre-match (before start) the
  prefix is `""`.
...
| `timing.timeTransitions["130000"] {layer, displayText, …}`                                        | `timing.phases[] {id: camelCase(displayText), label, startMs, layer: "layer-N", prefix}`                               | prefix = camelCase(label) when prefixing, else `""`; no transitions → one `match` phase |
...
| `teleopActive`/`teleopInactive` present                                                           | `timing.shifts` (25 s, from teleop start to 30 s, `maxIndex` 4) and `timing.endgame` (30 s)                            | from v5 client constants; **verify per season**                                         |
...
- **Shift and endgame constants** were copied from the 2026 client; a future season must set
  them in `timing.shifts`/`timing.endgame` by hand (the converter warns).
