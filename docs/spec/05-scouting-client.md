...
- **SC-2** Last-actions readout: the last three _distinct-run_ action ids in the queue,
...
- **SC-34 `climbHighlight`**: remove `.highlight` from every button that has a `climbHighlight`
  ...
  (**NOTE**: matches on the _prefixed_ action id versus un-prefixed button id, so the reverse
