...
executables["position"] = {
  execute(button, layers) {
    ...
    if (lockedTime + positionConfig.POSITION_LOCK_DELAY_MS > Date.now()) {
      ...
      setTimeout(() => {
        if (
          lockedTime + positionConfig.POSITION_LOCK_DELAY_MS <=
          Date.now() + 10
        )
          setAllButtonBorders(layers, "transparent");
    ...
    positionImage.addEventListener("click", (e) => {
      ...
      let aspectRatio =
        positionImage.naturalWidth / positionImage.naturalHeight;
...
function setAllButtonBorders(layers, color) {
  for (let button of layers.flat()) {
    if (button.executables.filter((x) => x.type == "position").length > 0) {
      //it has a position executable
