...
new DataTransformer("cycle", (dataset, outputPath, options) => {
  for (let tmp of dataset.tmps) {
    ...
    let endings = tmp.actionQueue.filter(
      (x) => options.scores.includes(x.id) || options.misses.includes(x.id)
    );
    ...
    out.averageTime =
      out.all.reduce((acc, x) => acc + x.timeDifferential, 0) / out.all.length;

    //exclude misses
    out.allComplete = out.all.filter(
      (x) => !options.misses.includes(x.ending.id)
    );
    out.averageTimeComplete =
      out.allComplete.reduce((acc, x) => acc + x.timeDifferential, 0) /
      out.allComplete.length;
