...
new DataTransformer("finalActionOccurrence", (dataset, outputPath, options) => {
  ...
  if (!options)
    throw new Error(
      "no options provided! Please provide an array of ids or set all to true"
    );
  ...
  for (let tmp of dataset.tmps) {
    for (let action of getPath(tmp, actionArrayPath).reverse()) {
      //look at every action in the action array backwards
      if (countedIds.includes(action.id)) {
        setPath(tmp, outputPath, action);
        break;
      }
    }
    if (!getPath(tmp, outputPath, false) && options.default) {
      //default object
