...
async function updateScouters(accessCode) {
  //scouter fetch interval (every 2.5s)
  let scouterList = await (
...
async function updateMatches(accessCode) {
  let { allMatches, currentMatch } = await (
  ...
  for (let match of allMatches) {
    ...
    if (currentMatch.match_string == match.match_string) {
      //check the box if it is selected
...
class ScouterDisplay {
  ...
  updateScouterElement(state) {
    ...
    //write all text
    this.scouterElement.querySelector(".scouter-id").innerText =
