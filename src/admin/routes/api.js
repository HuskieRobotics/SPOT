...
router.get("/auth", (req, res) => {
  if (!DEMO) {
    if (config.secrets.ACCESS_CODE === "") {
      res.json({ status: 2 });
    } else if (config.secrets.ACCESS_CODE == req.headers.authorization || `${req.headers.cookie}`.includes(`auth=${config.secrets.ACCESS_CODE}`)) {
...
router.get("/scouters", (req, res) => {
  if (!DEMO) {
    if (config.secrets.ACCESS_CODE == req.headers.authorization || `${req.headers.cookie}`.includes(`auth=${config.secrets.ACCESS_CODE}`)) {
